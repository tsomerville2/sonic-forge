#!/bin/bash
# VibeVoice Template Demo Player — arrow keys to browse, space to play
# Same captain's log lyrics, 8 different genre templates, VibeVoice TTS

VENV_PYTHON="/Users/t/clients/starship/starforge/venv/bin/python"
DEMO_DIR="$(cd "$(dirname "$0")" && pwd)"

rm -f /tmp/vv_demo_player_*.py
TMPSCRIPT="/tmp/vv_demo_player_$$.py"
...
TRACKS = [
    ("trance",    "136 BPM", "Pluck arps, acid, supersaw peak, breakdowns",     "Alice"),
    ("lofi",      " 78 BPM", "Jazzy chords, mellow plucks, light boom-bap",     "Alice"),
    ("cinematic", "100 BPM", "Tension build to massive hit, dramatic arc",       "Frank"),
    ("ambient",   " 68 BPM", "No beats, pure pads, vast slow atmosphere",        "Alice"),
    ("acid",      "132 BPM", "303 bass line is the star, minimal drums",         "Frank"),
    ("hiphop",    " 88 BPM", "Boom-bap beat, bass heavy, voice forward",         "Frank"),
    ("minimal",   "124 BPM", "Hypnotic repetition, subtle changes, deep groove", "Frank"),
    ("anthem",    "138 BPM", "Uplifting supersaws, triumphant builds, huge pads", "Alice"),
]
...
def main(stdscr):
    ...
    for i in range(len(TRACKS)):
        wav = os.path.join(DEMO_DIR, f"captains_log_{TRACKS[i][0]}.wav")
        if os.path.exists(wav):
            play_track(i)
            current = i
            break
    ...
    while True:
        ...
        # Title
        title = " VIBEVOICE DEMOS — Captain's Log "
        cx = max(0, (w - len(title)) // 2)
        safe_add(1, cx, title, curses.A_BOLD | curses.color_pair(1))

        subtitle = "Same lyrics, 8 genres, VibeVoice 1.5B neural TTS"
        ...
        for i, (name, bpm, desc, voice) in enumerate(TRACKS):
            ...
            elif is_selected and is_playing:
                marker = " >> "
            ...
            safe_add(y, 35, desc[:w-38], curses.color_pair(5) if exists else curses.A_DIM)
            y += 1
        ...
        if h > 3:
            safe_add(h - 2, 2,
                     "  arrows select   ENTER/SPACE play   q quit",
                     curses.color_pair(5))
